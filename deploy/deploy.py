...
class BlueGreenDeploy:
    """
    Currently this class only works for prod-antgirl-api.
    It does not work for antgirl-cron-prod or antgirl-consumer-prod
    """
    ...
    def get_latest_task(self) -> str:
        response = self.ecs_client.list_task_definitions(
            familyPrefix="prod-antgirl-api", status="ACTIVE", sort="DESC"
        )
        arns = response.get("taskDefinitionArns", [])
        return arns[0]

    def create_appspec_content(self, task_definition: str) -> Dict[str, str]:
    ...
    def create_deployment(self, app_spec_content: Dict[str, str]) -> str:
        deploy_result = self.code_deploy_client.create_deployment(
            applicationName="AppECS-prod-antgirl-api-cluster-prod-antgirl-api",
            deploymentGroupName="DgpECS-prod-antgirl-api-cluster-prod-antgirl-api",
