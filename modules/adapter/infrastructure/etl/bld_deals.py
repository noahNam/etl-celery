...
class TransferAptDeals(Transfer):
    ...
    ) -> tuple[list[AptDealModel], list[int]] | tuple[
        list[AptRentModel], list[int]
    ] | tuple[list[OfctlDealModel], list[int]] | tuple[
        list[OfctlRentModel], list[int]
    ] | tuple[
        ...
        if transfer_type == GovtFindTypeEnum.APT_DEALS_INPUT.value:
            ...
            for govt_apt_deal in tqdm(entities, desc="govt_apt_deal", mininterval=1):
                ...
                apt_dael = AptDealModel(
                    ...
                    private_area=MathHelper().round(
                        float(govt_apt_deal.exclusive_area), 2
                    ),
            ...
            for govt_apt_rent in tqdm(entities, desc="govt_apt_rent", mininterval=1):
                ...
                apt_rent = AptRentModel(
                    ...
                    private_area=MathHelper().round(
                        float(govt_apt_rent.exclusive_area), 2
                    ),
                    supply_area=supply_area,
            ...
            for govt_right_lot_out in tqdm(entities, desc="govt_apt_rent", mininterval=1):
                ...
                right_lot_out = RightLotOutModel(
                    ...
                    private_area=MathHelper().round(
                        float(govt_right_lot_out.exclusive_area), 2
                    ),
                    supply_area=supply_area,
    ...
    def _get_supply_area(
        ...
        for supply_entity in supply_areas:
            if (
                govt_deal.house_id == supply_entity.house_id
