...
class SyncBasicRepository(BasicRepository, BaseSyncRepository):
    ...
    def save(self, target_model: Type[BasicInfoModel | DongInfoModel | TypeInfoModel | MgmtCostModel], value: [BasicInfoModel | DongInfoModel | TypeInfoModel | MgmtCostModel]) -> None:
    ...
    def update(self, target_model: Type[BasicInfoModel | DongInfoModel | TypeInfoModel | MgmtCostModel], value: [BasicInfoModel | DongInfoModel | TypeInfoModel | MgmtCostModel]) -> None:
        with self.session_factory() as session:
            if target_model == BasicInfoModel:
                ...
                session.execute(
                    update(MgmtCostModel)
                        .where(MgmtCostModel.house_id == value.house_id and MgmtCostModel.payment_date == value.payment_date)
                        .values(
        ...
        with self.session_factory() as session:
            if isinstance(value, BasicInfoModel):
                ...
                query = (
                    ...
                    .where(TypeInfoModel.dong_id == value.dong_id, TypeInfoModel.private_area == value.private_area and TypeInfoModel.supply_area == value.supply_area)
                    .limit(1)
                )
                ...
                query = (
                    ...
                    .where(MgmtCostModel.house_id == value.house_id, MgmtCostModel.payment_date == value.payment_date)
                    .limit(1)
                )
