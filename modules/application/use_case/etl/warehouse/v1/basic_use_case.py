...
class BasicUseCase(BaseBasicUseCase):
    ...
    def execute(self):
        ...
        results: list[BasicInfoModel] = TransformBasic().start_etl(
        ...
        for kapt_mgmt_cost in kapt_mgmt_costs:
            kapt_mgmt_cost.house_id = self._basic_repo.find_house_id_by_code(
                kapt_code=kapt_mgmt_cost.kapt_code
            )

        results: list[MgmtCostModel] = TransformBasic().start_etl(
